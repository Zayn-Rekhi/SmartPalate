...
def get_meals(ingredients_str, ingredients_arr, meal_number = 20, **args):#make it a list of stuff as an input
    ...
    for meal in possibleMeals:
        ...
        reviews[meal["title"]] = 3 + random.random() * 2
        ...
        for nutrient in meal_nutrition:
            ...
            if name == "Calories" or name == 'Fat' or name  == 'Carbohydrates' or name == 'Protein':
                meal_nutrition_ret[meal["title"]][name] = nutrient["amount"]#/servings
    
    return meal_names_ret, meal_ids_ret, ingredientsRet, meal_images_ret, meal_nutrition_ret, reviews
...
def getMetaData(ingredients, nutrition, recipe_id, review):
    ...
    metadata = {
        ...
        'instructions': instructions[4:-5].replace("</li>", "").split("<li>")[1:],
